...
Record full-task duration and host-measured counters, including retrieval, verification, and all retries; do not submit only the successful final attempt. Reports compare `durationMs` and the portable token/tool/retry metrics above, with candidate-minus-baseline deltas. A metric absent on either side of any pair is explicitly `unavailable` for that split, never zero or a partial total. Word counts cannot stand in for token/cost savings. Cached input tokens are reported separately, not added to input tokens; context peak totals describe the sum of per-run peaks, not concurrent memory usage. Interpret fewer tokens alongside retries, latency, and quality, not in isolation.
