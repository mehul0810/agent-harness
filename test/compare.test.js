...
test('rejects mismatched artifacts, leakage, duplicate runs, bad telemetry and open payloads', () => {
  for (const mutate of [
    ...
    (x) => { x.pairs[0].candidate.durationScope = 'partial'; },
...
test('excludes elapsed timings unless every paired run is a full-task duration', () => {
  for (const scope of ['partial_observation_window', 'shared_batch_interval', undefined]) {
    const input = fixture();
    input.pairs[0].candidate.durationScope = scope;
    const result = compareRuns(input);
    assert.equal(result.valid, true);
    assert.deepEqual(result.splits['held-out'].metrics.durationMs, { status: 'unavailable' });
  }
  const fullTask = fixture();
  assert.deepEqual(compareRuns(fullTask).splits['held-out'].metrics.durationMs, { status: 'available', baseline: 100, candidate: 100, delta: 0 });
});
